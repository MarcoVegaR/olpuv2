...
export default function IndexPage() {
    ...
    const [density, setDensity] = React.useState<'comfortable' | 'compact'>(() => {
        ...
        const saved = window.localStorage.getItem('requirement_table_density');
        return saved === 'compact' ? 'compact' : 'comfortable';
    });

    const permissions = {
        canCreate: auth?.can?.['catalogs.requirement.create'] || false,
        canEdit: auth?.can?.['catalogs.requirement.update'] || false,
        canDelete: auth?.can?.['catalogs.requirement.delete'] || false,
        canExport: auth?.can?.['catalogs.requirement.export'] || false,
        canBulkDelete: auth?.can?.['catalogs.requirement.delete'] || false,
        canSetActive: auth?.can?.['catalogs.requirement.setActive'] || false,
        canBulkSetActive: auth?.can?.['catalogs.requirement.setActive'] || false,
    };
    ...
    const reloadData = React.useCallback(() => {
        ...
        router.get('/catalogs/requirement', params, {
            only: ['rows', 'meta'],
            preserveState: true,
            preserveScroll: true,
        });
    ...
    const handleExport = React.useCallback(
        (format: string = 'csv') => {
            ...
            window.location.href = `/catalogs/requirement/export?${usp.toString()}`;
        },
        [pageIndex, pageSize, globalFilter, sorting],
    );
                ...
                <div className="py-8">
                    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                        ...
                        <div className="mb-8 flex items-center justify-between">
                            <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Requisitos</h1>
                            {permissions.canCreate && (
                                <Link href="/catalogs/requirement/create">
                            ...
                            <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Total Requisitos</p>
                                ...
                                <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Requisito Activos</p>
                        ...
                        <div className="overflow-hidden rounded-xl bg-white shadow-lg ring-1 ring-gray-200 dark:bg-gray-800 dark:ring-gray-700">
                            <div className="p-6">
                                <DataTable
                                    ...
                                    onDensityChange={(d) => {
                                        setDensity(d);
                                        if (typeof window !== 'undefined') window.localStorage.setItem('requirement_table_density', d);
                                    }}
